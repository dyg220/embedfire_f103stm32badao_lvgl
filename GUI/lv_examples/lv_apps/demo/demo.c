...
//  lv_obj_t *slider1 = lv_slider_create(lv_disp_get_scr_act(disp), NULL);
//  lv_slider_set_value(slider1, 70, LV_ANIM_ON);
//  lv_obj_set_pos(slider1, 10, 10);

  /*Createa Tab viewobject*/
  lv_obj_t *tabview;
  tabview = lv_tabview_create(lv_scr_act(), NULL);
  /*Add3 tabs(thetabsarepage(lv_page) andcanbescrolled*/
  ...
  lv_label_set_text(label, "This thefirst tab\n\n"
                           "Ifthe content\n"
                           "ofa tab\n"
                           "become too long\n"
                           "theit \n"
  ...
  lv_label_set_text(label, "Thirdtab");
}
...
static void write_create(lv_obj_t *parent) {
  ...
  style_kb_rel.body.main_color = lv_color_hex3(
      0x333); /*Recommended if LV_VDB_SIZE == 0 and bpp > 1 fonts are used*/
...
static void text_area_event_handler(lv_obj_t *text_area, lv_event_t event) {
  ...
  if (event == LV_EVENT_CLICKED) {
    if (kb == NULL) {
      ...
      lv_obj_set_size(kb, lv_obj_get_width_fit(parent),
...
static void chart_create(lv_obj_t *parent) {
  ...
  lv_obj_align(slider, chart, LV_ALIGN_OUT_BOTTOM_MID, 0,
               (vres - chart->coords.y2 - lv_obj_get_height(slider)) /
                   2); /*Align to below the chart*/
  lv_obj_set_event_cb(slider, slider_event_handler);
  lv_slider_set_range(slider, 10, 1000);
  lv_slider_set_value(slider, 700, false);
  slider_event_handler(slider,
                       LV_EVENT_VALUE_CHANGED); /*Simulate a user value set the
                                                   refresh the chart*/
}
